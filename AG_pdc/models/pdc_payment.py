...
class AccountPayment(models.Model):
    ...
    cheque_state = fields.Selection(selection=[
            ...
            ('cancel', 'CANCELLED'),
            ('bounce', 'BOUNCED'),
        ], default= 'draft', string='Status', readonly=True, copy=False, tracking=True)
    ...
    def cheque_entry_view(self):
        for each1 in self:
            ...
            return {
            'name': _('Cheque Entries'),
            'domain': [('cheque_payment_id', '=', self.id)],
    ...
    def action_draft(self):
        ...
        self.write({'cheque_state':'draft','second_approval_done':False})
    ...
    def release_button(self, write_off_line_vals=None):
        ...
        if not self.release_move:
            ...
            if self.payment_difference:
                ...
                self.write({'release_move':release_move.id,'cheque_state':'deposit'})
            else:
                for line in self.move_id.line_ids:
                # line_vals_list = [
                    # Liquidity line.
    ...
    def deposit_button(self):
        line_vals_list = []
        vals_deb = {
            'name': 'Deposit PDC No.%s'%self.cheque_reference,
    ...
    def return_cheque(self):
        ...
        vals_deb = {
            ...
            'partner_id': self.partner_id.id,
            #'cheque_payment_id': self.id,
            'account_id': self.journal_id.pdc_account.id,
        }
        line_vals_list.append((0,0,vals_deb))
