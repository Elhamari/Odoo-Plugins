...
class FinalSettlement(models.Model):
	...
	def action_generate(self):
		...
		for obj in self:
			...
			contract_type_name = obj.employee_id.contract_id.contract_type
	...
	def action_final_settlement(self):
		...
		for obj in self:
			...
			final_pay = (final_due_amt + obj.fs_gross_available_days + obj.total_add_amt + obj.pending_leave_sal) - obj.total_dec_amt
	...
	@api.depends('pending_date_from','pending_date_to','fs_gross_per_pending_day','total_salary')
	def action_calc_pending_sal(self):
		...
		if p1 and p2:
			...
			self.fs_gross_per_pending_day = self.total_salary / days_of_month
